...
public:
  ...
  EMC2301_STATUS setFanSpeedMin(uint16_t minRPM);
  ...
  EMC2301_STATUS setFanMinDrive(uint8_t minDriveByte);
  EMC2301_STATUS setFanSpeedSpinupMin(uint16_t minRPM);
  EMC2301_STATUS setFanSpeedTarget(uint16_t targetRPM);
...
private:
  ...
  const double TACHO_FREQUENCY = 32768;
  ...
  */
  ...
  uint8_t fanEdgesCount_;
  uint8_t fanPoleCount_;
  unsigned long tachoRPMConstant_;
  uint16_t targetTachCount_;
  uint16_t fanSpeed_;

  void recalculateTachoRPMConstant();
